# Focus
In this project, I will introduce how to reproduce GPT-3 training, with a focus on writing CUDA kernels. The tutorial will cover everything from foundational XYZ dimensions to PTX (Parallel Thread Execution) and SASS (Streaming Assembler).

# C++ -> PTX -> SASS
![pipeline](README/images/pipeline.png)

When we write CUDA C++ code, which includes both host (CPU) and device (GPU) code, it is first processed by the compiler nvcc. The device-specific code (kernels) is compiled into PTX, which is an intermediate assembly-like language designed by NVIDIA. PTX serves as a portable, hardware-agnostic representation of your GPU code, allowing for optimizations and future GPU architecture support without recompiling the high-level CUDA code. The PTX code is then either just-in-time (JIT) compiled by the NVIDIA driver at runtime into SASS, which is the actual machine code executed on the GPU. This is important to remember because further CUDA code optimizations will proceed through this pipeline.

PTX Generation: The device-specific code (kernels) is compiled into PTX, which is an intermediate assembly-like language designed by NVIDIA. PTX serves as a portable, hardware-agnostic representation of your GPU code, allowing for optimizations and future GPU architecture support without recompiling the high-level CUDA code.
Compilation from PTX to SASS (Streaming Assembler)/

SASS Generation: The PTX code is then either just-in-time (JIT) compiled by the NVIDIA driver at runtime into SASS, which is the actual machine code executed on the GPU, or it can be pre-compiled into SASS during the build process if you target specific GPU architectures.
Execution: The SASS code is what the GPU hardware executes directly.

## GPU Kernel Concepts
### 1. One-Dimensional Native Kernel: initialize_norm_kernel
### 2. Two-Dimensional Native Kernel
### 3. Global Memory Coalescing
### 4. Shared Memory Cache
...

## Part
first part: allocate memory and initilize weights (this will copy openai GPT2 initialize strategy)

second part: forward

three part: backward

four part: data prepare

## Why i create this project?
When I first wanted to learn CUDA, I tried to understand the karpathy's llm.c code. However, it was incredibly challenging without a foundational understanding of CUDA. I also explored Elliotcodes' CUDA course, which provided valuable insights into principles and example code. However, I found myself spending a lot of time learning concepts without seeing how they connected to a practical project.
From my past experiences, I realized that a project-based approach is the fastest and most effective way to grasp a new language or concept. That’s when I thought, "Why not create an introductory CUDA course centered around an engaging, medium-sized project?"
